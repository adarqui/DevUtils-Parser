module System.DevUtils.Parser.KV.String (
 defaultKV,
 runKV,
 weirdKV
) where
...
runKV' :: KV -> St [Pair] -> String -> Either String [Pair]
runKV' kv p input = do
 case (runParser p kv "KV" input) of
  Left err -> Left $ "Parser error: " ++ show err
  Right val -> Right val

runKV :: KV -> String -> Either String [Pair]
